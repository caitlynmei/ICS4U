...
public class Graph {
	private final int V;
	private int E;
	private Bag<Integer>[] adj; // array of bag of integers 
	...
	public Graph(int rows, int cols, boolean allowAdjacent, boolean allowDiagonals, boolean allowKnights) {
		...
		// for a 2D graph 
		if (rows < 0 || cols < 0)
			throw new IllegalArgumentException("Rows and Cols of grid must be nonnegative");
		...
		// if allowDiagonals is false || true then we can only go up down left and
		// right.
		if (allowAdjacent) {
	...
	public Graph(int x, int y, int z, boolean allowAdjacent, boolean allowDiagonals, boolean allowKnights) {
		// booleans for how to connect vertices 
		
		if (x < 0 || y < 0 || z < 0) // 3D graph; a cube 
			throw new IllegalArgumentException("x, y and z of grid must be nonnegative");
		...
		if (allowAdjacent) {
			...
			for (int j = 0; j < z; j++) {
				...
				for (int i = 0; i < x * y; i++) {
					if ((i + 1) % y != 0)
						addEdge(i + translation, i + translation + 1); // adds an edge to the right and vice versa
																		// (East)
	...
	public void disconnectVertex(int v) {
		...
		// go through the vertex - and remove all of the edges edges connecting it to
		// some other vertex.
